...
def generation(trajectron_g, trajectron, node_type, device, inputs, seed_targets, targets, p_accept,
    ...
    for _ in range(max_iter):
        ...
        loss = criterion(outputs_g, targets) + lam * \
            classwise_loss(outputs_r, seed_targets)
        # x, n_s_t0, x_nr_t
...
def train_net(trajectron, trajectron_g, node_type, criterion, optimizer, lr_scheduler, inputs_orig_tuple,
    ...
    inputs_tuple_ = tuple(
        tensor.clone() if tensor is not None else None for tensor in inputs_orig_tuple)
    ...
    ########################
    bs = N_SAMPLES_PER_CLASS_T[targets_orig].repeat(
        gen_idx.size(0), 1)  # [gen_idx, BS]
...
def train_gen_epoch(trajectron, trajectron_g, curr_iter_node_type, optimizer, lr_scheduler, criterion,
    ...
    for node_type, data_loader in train_data_loader.items():
        ...
        for batch in pbar:
            ...
            e_x = trajectron.encoded_x(x, node_type)
            e_x = tuple(
                tensor.detach() if tensor is not None else None for tensor in e_x)
