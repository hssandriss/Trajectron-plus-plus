...
def compute_batch_statistics_classes(prediction_output_dict,
                             dt,
                             current_epes,
                             nb_classes,
                             max_hl,
    ...
    (prediction_dict,
     ...
     futures_dict) = prediction_output_to_trajectories(prediction_output_dict,
                                                       ...
                                                       prune_ph_to_future=prune_ph_to_future)
    batch_error_dict = dict()
    batch_error_dict_details = dict()
    for node_type in node_type_enum:
        batch_error_dict[node_type] =  {'ade': list(), 'fde': list(), 'kde': list(), 'obs_viols': list()}
    for node_type in node_type_enum:
        batch_error_dict_details[node_type] = dict()
        for i in range(nb_classes):
            batch_error_dict_details[node_type][i] =  {'ade': list(), 'fde': list(), 'kde': list(), 'obs_viols': list()}
    current_epe_index = 0
    for t in prediction_dict.keys():
        for node in prediction_dict[t].keys():
            ade_errors = compute_ade(prediction_dict[t][node], futures_dict[t][node])
            ...
            batch_error_dict_details[node.type][current_epes[current_epe_index]]['ade'].extend(list(ade_errors))
            batch_error_dict_details[node.type][current_epes[current_epe_index]]['fde'].extend(list(fde_errors))
            batch_error_dict_details[node.type][current_epes[current_epe_index]]['kde'].extend([kde_ll])
            batch_error_dict_details[node.type][current_epes[current_epe_index]]['obs_viols'].extend([obs_viols])
            current_epe_index += 1
    #import pdb; pdb.set_trace()
    return batch_error_dict, batch_error_dict_details
